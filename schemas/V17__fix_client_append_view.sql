...
SELECT
    ...
    COALESCE(client_append.enable_individual_landline, append.enable_individual_landline) enable_individual_landline,
    COALESCE(client_append.enable_household_landline, append.enable_household_landline) enable_household_landline,
    COALESCE(client_append.enable_contact_point_landline, append.enable_contact_point_landline) enable_contact_point_landline,
    COALESCE(client_append.enable_individual_wireless, append.enable_individual_wireless) enable_individual_wireless,
    COALESCE(client_append.enable_household_wireless, append.enable_household_wireless) enable_household_wireless,
    COALESCE(client_append.enable_contact_point_wireless, append.enable_contact_point_wireless) enable_contact_point_wireless
FROM ${schemaName}.append
         CROSS JOIN ${schemaName}.client
         LEFT JOIN ${schemaName}.client_append
            ON client_append.append_id = append.id
            AND client.id = client_append.client_id
WHERE (client_append.id IS NOT NULL AND client_append.enabled)
   OR (client_append.id IS NULL AND append.enabled)
